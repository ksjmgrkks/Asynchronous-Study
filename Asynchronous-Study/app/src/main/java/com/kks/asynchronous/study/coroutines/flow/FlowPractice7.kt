...
fun main() = runBlocking<Unit> {
    ...
    /* 예제에서는 적합한 코드는 아닙니다. 하지만 데이터가 짝을 이룰 필요없이 최신의 데이터를 이용해 가공해야 하는 경우에 사용할 수 있습니다.
       안드로이드에서 예시를 생각해보면, 2개의 api 호출 결과를 묶어 데이터를 새로 생성할 때는 zip을 이용하고,
       최신 UIState를 업데이트 해줘야 하는 상황에서는 굳이 두 데이터 스트림이 짝을 이룰 필요는 없고,
       하나의 데이터 스트림이라도 emit이 되면 UI 업데이트를 하면 되므로 combine이 적절해 보입니다.
